...
$objdata = new consulta($con);
	    ...
	    $sql= "SELECT idinterno,dataemissao,numnotafiscal,idexterno,idtipomovimento,idmotivo,st,dataentrega,numconta,
		       quantidadevolumes,pago,dataoperacao,numlotecliente,primeiroenvelope,numerosedex FROM tbentrega 
	...
	if ($tot_cont > 0) {
			...
			for ($i=0;$i<$qry1->nrw;$i++){
				...
				$qry5->executa("select idexterno,idinterno,dataentrega,idmotivo,st from tbentrega
				...
				if ($qry1->data["st"]== 'D' or $qry1->data["st"]== 'N')
				{
					...
					$qry8->executa("select nome_ocorrencia from tb_sabo_ocorr
					where id_ocorrencia=".$qry5->data["idmotivo"]);
					$mot = $qry8->data["nome_ocorrencia"];
