	  ...
	  $codbars = trim($codbars);
		...
		$qry->executa("Select pago,idtransportadora,codcliente,codigoproduto,
                   idinterno,codbase,idtipomovimento,numerosedex,
		       	       numlista,cpf, datacoletado from tbentrega where idexterno='".$codbars."'");
	 ...
	 if($op==2 || $op==18){
	  ...
	  <?php
		...
		if ($err[0]>0){
			 ...
			 if($radio==2){
			 	echo "<p>".$codbars.": <font color=#990000>Movimento n&atilde;o permitido.$obs / $idtipomovimento_atual 170</font></p>";
			 }else{
			 	echo "<p>".$codbars.": <font color=#990000>Movimento n&atilde;o permitido.$obs / $idtipomovimento_atual ".$qry1->data["idtipomovimento"]."</font></p>";
			 }
		...
		if ($err[13]>0){
			echo "<p><font color=#990000>ATENÇÃO, RA EM BLACKLIST.</font></p>";
			echo "<script>
				$(function() {
					$('#myModalb').modal('show');
				});
...
$qry12->executa("SELECT id FROM tb_easy_courier WHERE (nr_encomenda = '$cpf')");
...
<div class="modal fade" id="myModalb" tabindex="-1" role="dialog" aria-labelledby="myModalLabelb">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabelb">ATENÇÃO, RA EM BLACKLIST.</h4>
      </div>
    </div>
  </div>
</div>
